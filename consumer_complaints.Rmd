...
knitr::opts_chunk$set(dev = 'pdf', echo=FALSE, warning=FALSE, message=FALSE)
...
The time series plot once again confirms that the South has the largest number of complaints over time. However, it is very interesting that the plots of all regions show to have a split with two branches. I suspect one of these branches belong to the mortgage complaints while the other branch is for the other projects. However, further investigation will be needed to confirm this suspicion. 

#### Maping the number of complaints by state
...
In this study, I wanted to find out the qualities of different services or producted offered in the States by investigating the number of complaints that were filed. I investigated how many of these complaints have delayed response from the company of the service. I started by filtering out the dataset, which was obtained from the indicated linked in the introduction section, by omitting all the datapoints that don't have a valid state. I generated a new dataframe to see which products or services have the largest number of complaints. It turns out that Mortgage services/products have the highest number of complaints files. In addition, I also grouped the number of complaints by Company to list out the top 20 companies with the highest number of complaints. 

By creating a bar plot, which focused on the mortgage section of the data, I found out that Residential Credit Solutions has the largest number of delayed responses to their customers'complaints. In addition, I also generated two choropleths: one shows the distribution of the number of complaints for all products by State and one shows the distribution of the number of complaints for mortgage products by State. 

The main ideas of this study is to help customers have a better idea when they come to choose a particular product and to help companies with the correponding product visualize the statistics and improve their service to the customers. 